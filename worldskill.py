...
with open('Session1_DataExploration.txt', 'w') as f:
    f.write("Отчет по исследованию данных\n\n")
    
    f.write("Примеры данных из датасетов:\n\n")
    f.write("Продажи:\n")
    df_sales.head(5).to_string(f, header=True, index=False)
    f.write("\n\n")

    f.write("Продукты:\n")
    df_products.head(5).to_string(f, header=True, index=False)
    f.write("\n\n")

    f.write("Клиенты:\n")
    df_customers.head(5).to_string(f, header=True, index=False)
    f.write("\n\n")
...
for cust_id in cust_features.index:
    ...
    # ИСПРАВЛЕНИЕ: Гарантируем, что список recs содержит 3 элемента, дополняя его 'NA'
    while len(recs) < 3:
        recs.append('NA')
